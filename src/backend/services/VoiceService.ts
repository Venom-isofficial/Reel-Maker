...
export class VoiceService {
  ...
  public async getExactAudioDuration(filePath: string): Promise<number> {
    try {
  ...
  private async applySpeedFilter(filePath: string, speedMultiplier: number = 1.15): Promise<void> {
    ...
    try {
      ...
      console.warn('ffmpeg speed adjustment error, using original audio:', err.message);
    }
  }
  ...
  public async generateVoice(
    ...
    exaggeration?: number,
    cfgWeight?: number
  ): Promise<ServiceResult<{ audioPath: string; duration: number; provider?: string }>> {
    try {
        ...
        try {
          ...
          if (fs.existsSync(outputMp3Path) && fs.statSync(outputMp3Path).size > 1000) {
            ...
            return {
              success: true,
              retryable: false,
              data: { audioPath: outputMp3Path, duration, provider: 'chatterbox' },
            };
          } else {
            throw new Error(`Chatterbox synthesis file was empty or missing at ${outputMp3Path}`);
          }
        } catch (cbErr: any) {
          console.error('Official Chatterbox 500M TTS error:', cbErr.message);
          return {
            success: false,
            retryable: true,
            errorMessage: `Chatterbox 500M TTS error: ${cbErr.message}`,
          };
        }
      }
        ...
        if (fs.existsSync(pythonScript)) {
          ...
          if (fs.existsSync(outputMp3Path) && fs.statSync(outputMp3Path).size > 1000) {
            ...
            return {
              success: true,
              retryable: false,
              data: { audioPath: outputMp3Path, duration, provider: 'kokoro' },
            };
          }
        }
      } catch (kokoroErr: any) {
